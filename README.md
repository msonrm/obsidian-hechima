...
- **打ちながら候補が出ます**（「変換」を押す前から）。タップするとその語で確定し、
  よみが残っていればそのまま続きの候補が出ます
- 「変換」を押したあとの候補も同じ帯に出ます。**タップするとその文節が確定**し、
  続きに未変換の文節があればそのまま候補が出ます（横に流せばスクロール）
- 薙刀式や NICOLA を選んだままでも使えます（フリックの操作キーは配列を通りません）
- **タブレットでは英字キーボード（QWERTY）が隣に並びます**。英単語・URL・コード片のような
  「かなを通さないもの」を直接打つための面です（縦向きなら左右、横向きなら上下。
  設定で常に出す / 出さないも選べます）
