...
class HechimaIME {
    ...
    roleCode(role, id = this.keymapId) {
        const override = this.plugin.settings.roleKeys?.[id]?.[role];
    ...
    setKeymap(id) {
        ...
        // 配列ごとの割り当て（v0.12.2〜）。役名が機能ベースに統一されたため、
        // グローバルに持つと薙刀式と NICOLA の holder1 が混ざる
        const overrides = this.plugin.settings.roleKeys?.[id] ?? {};
     ...
     */
    flickKey(tap) {
        // **変換中の ←→ は文節の伸縮**（タッチ入力の作法。iOS / Android の日本語入力と同じ）。
        // 文節の「移動」は要らない —— 候補をタップすればその文節が確定して次へ進むので
        // （tapCandidate）、注目を左右に動かす操作が出番を持たない。
        // 物理キーボードは従来どおり ←→ が移動 / Shift+←→ が伸縮。
        if (
            !tap.shiftKey && (tap.key === "ArrowLeft" || tap.key === "ArrowRight") &&
            this.hasCandidates()
        ) {
            tap = { ...tap, shiftKey: true };
        }
    ...
    hasCandidates() {
        return !!this.lastSegments?.some((s) => s.kind === "focus" && s.candidates?.length);
    }
...
@media (orientation: landscape) {
  ...
  /* 横持ちは**上下に積む**。左右の入替（data-side）はパネルの位置に効くので、
     面の順序には効かせない。★**包みも伸ばさない**（flex: 0 0 auto）——
     伸びると下の margin-bottom: auto が効かず、塊が上に張り付く。
     中央寄せはこれで 2 度壊れている（0.16.0 = 後勝ち / 0.20.0 = 包みを挟んだとき） */
  .hechima-flick-faces { flex: 0 0 auto; flex-direction: column; gap: 16px; }
  .hechima-flick[data-side="left"] .hechima-flick-faces { flex-direction: column; }
  /* 各面は**幅から高さが決まる**。伸ばさない・縮めない —— 伸ばすと画面の縦幅を
     全部占拠する。英字面は 10 列 4 行だが、**比率どおりだとキーが平たくなりすぎる**
     ので縦を 1.5 倍に取る（実機の指摘。10/4 → 10/6） */
  .hechima-flick-area { flex: 0 0 auto; aspect-ratio: 5 / 4; max-height: 100%; }
  .hechima-flick-area2 { flex: 0 0 auto; aspect-ratio: 10 / 6; max-height: 100%; }
...
class FlickPanel {
    ...
    async build() {
        ...
        // 端末の種別は幅の分岐に**使わない**（一時期 desktop だけ細くしていたが、
        // iPad 横持ちも同じく大きすぎた = 分けるべきなのは端末ではなく「画面の広さ」で、
...
const { Plugin, Notice, Modal, Setting, PluginSettingTab, Scope, Platform, apiVersion, requestUrl } = require("obsidian");
...
module.exports = class HechimaProbePlugin extends Plugin {
    ...
    async onload() {
        ...
        // 役の正規名も機能ベースになった（leftThumb → holder1）。あわせて **roleKeys を
        // 配列ごと**に移す —— v2 では薙刀式の space も holder1 になるので、グローバルに
        // 持つと NICOLA の左親指と衝突する。
        // 旧形式は { leftThumb: "NonConvert" }、新形式は { nicola: { holder1: "NonConvert" } }
        const roleRename = { leftThumb: "holder1", rightThumb: "holder2", space: "holder1" };
        ...
        if (legacy.length) {
            ...
            // **いま選んでいる配列にだけ**移す。どの配列向けの設定だったかは分からないので、
            // 全部に配ると他の配列を壊す
            rk[target] = { ...moved, ...(rk[target] ?? {}) };
            this.settings.roleKeys = rk;
        }
...
class HechimaSettingTab extends PluginSettingTab {
    ...
    async assignRole(role, code) {
        // **配列ごとに持つ。** keymap v2 で役名が機能ベースに統一されたので、
        // 薙刀式の holder1（スペース）と NICOLA の holder1（左親指）が同じ名前になった。
        // グローバルに持つと片方の割り当てがもう片方に漏れる
        const id = this.plugin.settings.keymapId;
        this.plugin.settings.roleKeys ??= {};
        this.plugin.settings.roleKeys[id] ??= {};
        if (code) this.plugin.settings.roleKeys[id][role] = code;
        else delete this.plugin.settings.roleKeys[id][role];
        await this.plugin.saveSettings();
     ...
     *
     * **OS が奪うキーはここにも来ない** —— 押しても反応しなければ、そのキーはこの端末では
     * 使えないということがその場で分かる（iPadOS の 英数 / かな がこれ）。それを言うには
     * 「届かなかった」と「そもそも待ち受けていなかった」を区別できないといけないので、
     * 時間切れは黙って戻さず Notice で知らせる。
     *
     * 網を 3 枚張る。設定画面はモーダルの中にあり、誰が先にキーを取るかが環境依存だから:
     *   1. Obsidian の Scope（ホットキー dispatch の最前段。ここが本命）
     *   2. window の capture（document より先に走る）
     *   3. document の capture（1・2 が使えない環境の保険）
     * 最初に取れたものが `done` を立てて残りを畳む。
     */
    captureKey(role, button) {
        if (this.capturing) this.capturing(); // 別の取得中なら畳む
        const IDLE = "キーを押して割り当て";
        ...
        const take = (e) => {
            ...
            if (!code || code === "Unidentified") {
                // Android の逐次系経路など、押下が合成イベントで届く環境
                new Notice("このキーは識別できません。この端末では役に使えません");
                return false;
            }
            new Notice(`${ROLE_LABELS[role] ?? role} ← ${code}`);
            void this.assignRole(role, code);
            return false; // Scope 経路: false = 既定動作を止める
        };
        ...
        const stop = (timedOut) => {
            ...
            clearTimeout(timer);
            try { window.removeEventListener("keydown", take, true); } catch { /* 無い環境 */ }
            try { document.removeEventListener("keydown", take, true); } catch { /* 同上 */ }
            try { if (scope) this.plugin.app.keymap?.popScope?.(scope); } catch { /* 同上 */ }
            this.capturing = null;
            button.setButtonText(IDLE);
            if (timedOut) new Notice("キーが届きませんでした（8 秒）。この端末では使えないキーかもしれません");
        };

        try {
            if (typeof Scope === "function" && this.plugin.app.keymap?.pushScope) {
                scope = new Scope();
                scope.register(null, null, take);
                this.plugin.app.keymap.pushScope(scope);
            }
        } catch { scope = null; }
        try { window.addEventListener("keydown", take, true); } catch { /* 無い環境 */ }
        try { document.addEventListener("keydown", take, true); } catch { /* 同上 */ }

        const timer = setTimeout(() => stop(true), 8000);
        this.capturing = stop;
    }
    ...
    display() {
        ...
        if (roles.length) {
            ...
            for (const role of roles) {
                ...
                const assigned = this.plugin.settings.roleKeys?.[this.plugin.settings.keymapId]?.[role];
                const note = ROLE_NOTE.get(assigned ?? "") || "";
                new Setting(containerEl)
                    ...
                    .addDropdown((d) => {
                        ...
                        d.setValue(this.plugin.settings.roleKeys?.[this.plugin.settings.keymapId]?.[role] ?? "");
